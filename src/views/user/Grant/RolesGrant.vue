...
import {
  ...
  allRolesList, userRolesDelete, userRolesAdd
} from "@/api/roles.js";
...
const UserRolesList = async () => {
  ...
  for (let i = 0; i < userRoles.value.length; i++) {
    ...
    for (let j = 0; j < roles.value.length; j++) {
      if (String(userRole.roleId) === String(roles.value[j].roleId)) {
        userRole.userRoleName = roles.value[j].roleName;
      }
    }
  }
}
...
const clearData = () => {
  userRolesModel.value.userId = userId.value
  userRolesModel.value.roleId = ''
  ...
  <el-card class="page-container">
    <template #header>
      <div class="header">
        <span>用户管理/分配角色</span>
        <div class="extra">
          <el-button type="primary" @click="dialogVisible = true,title='分配角色',clearData()">分配角色</el-button>
        </div>
      </div>
    </template>
    ...
    <el-form inline>
      ...
      <el-form-item>
        ...
        <el-button @click="roleId='';">重置</el-button>
      </el-form-item>
    </el-form>
    <!-- 文章列表 -->
    <el-table :data="userRoles" style="width: 100%">
      <el-table-column label="编号" width="100" prop="userRoleId"></el-table-column>
      <el-table-column label="用户名称">{{route.query.username}}</el-table-column>
      <el-table-column label="角色名称" prop="userRoleName"></el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="title" width="30%">
      <el-form :model="userRolesModel" label-width="100px" style="padding-right: 30px">
        <el-form-item label="用户编号">
          <el-input v-model="userRolesModel.userId" disabled></el-input>
        </el-form-item>
        <el-form-item label="角色名称">
          <el-select placeholder="请选择" v-model="userRolesModel.roleId">
