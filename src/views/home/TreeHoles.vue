...
import {addTreeHole, getTreeHoleList} from "@/api/TreeHoles.js";
...
const addTreeHoleBtn = async () => {
  ...
  treeHolesModel.value.content = content.value
  treeHolesModel.value.createrPic = userInfoStore.info.userPic
...
const getTreeHole = async () => {
  let result = await getTreeHoleList()
  treeHoleList.value = result.data
}
...
<template>
  <div class="container">
    <div class="video-background">
      ...
      >
        <source src="@/assets/【哲风壁纸】二次元-动态壁纸.mp4" type="video/mp4" />
        您的浏览器不支持视频播放。
      </video>
      <video
          v-else
          ...
          class="background-video"
      >
        <source src="@/assets/【哲风壁纸】崩坏：星穹铁道-星空背景.mp4" type="video/mp4" />
        您的浏览器不支持视频播放。
      </video>
    </div>
    ...
    <div class="content_container">
      ...
      <div>
        ...
        <button v-show="isShowSubmit" @click="addTreeHoleBtn(),clearData()"><span
            style="color: grey;font-style: italic;font-weight: bold">提交</span></button>
      </div>
    </div>
    <vue-danmaku :debounce="3000"
      ...
      <template v-slot:dm="{ danmu }">
        <div class="barrage_container">
          <div>
            <el-avatar :src="danmu.createrPic"/>
          </div>
          <div><span>{{ danmu.creater }}：</span><span>{{ danmu.content }}</span></div>
        </div>
      </template>
    </vue-danmaku>
  </div>
